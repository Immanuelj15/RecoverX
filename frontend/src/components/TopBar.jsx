...
export default function TopBar({ activeItem, activeTab, setActiveTab, recoveryState }) {
  ...
  return (
    <>
      <header className="h-[72px] bg-white border-b border-slate-200 flex items-center justify-between px-8 sticky top-0 z-40 shadow-sm">
        
        {/* Left: Breadcrumb / Title */}
        <div className="flex items-center gap-3">
          <div className="h-6 w-1 rounded-full bg-blue-600"></div>
          <h2 className="text-base lg:text-lg font-black text-slate-900 tracking-tight">{getBreadcrumb()}</h2>
        </div>
        ...
        <div className="flex-1 max-w-md mx-6 relative hidden md:block">
          <button
            ...
            className="w-full h-10 px-3.5 border border-slate-200 rounded-xl bg-slate-50 text-slate-600 text-xs lg:text-sm font-medium transition-all flex items-center justify-between hover:border-blue-500 hover:bg-white hover:text-slate-900 focus:outline-none cursor-pointer group shadow-inner"
          >
            <div className="flex items-center gap-2 min-w-0 pr-2">
              <Search className="h-4 w-4 text-slate-400 group-hover:text-blue-600 shrink-0" />
              <span className="truncate whitespace-nowrap text-slate-500 group-hover:text-slate-700">Search payment IDs, customers, SHAP drivers...</span>
            </div>
            <kbd className="shrink-0 text-slate-500 text-[10px] font-bold px-1.5 py-0.5 border border-slate-200 rounded-md bg-white font-mono shadow-sm">⌘K</kbd>
          </button>
        </div>
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="hidden lg:flex flex-col text-right">
            <span className="text-xs font-black text-slate-900">RecoverX Demo Store</span>
            <span className="text-[10px] text-emerald-600 font-mono font-semibold">Razorpay Gateway Sandboxed</span>
          </div>
          ...
          <div className="relative" ref={dateMenuRef}>
            <button
              ...
              className="hidden lg:flex items-center gap-2 px-3 py-1.5 rounded-xl border border-slate-200 bg-slate-50 text-xs text-slate-800 font-bold hover:border-blue-500 hover:bg-white transition-all cursor-pointer shadow-sm"
            >
            ...
            {showDateMenu && (
              <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-xl border border-slate-200 py-2 z-50 animate-fade-in text-slate-800">
          ...
          <div className="relative" ref={notifMenuRef}>
            <button
              ...
              className="relative p-2 text-slate-600 hover:text-slate-900 transition-colors rounded-xl hover:bg-slate-100 border border-transparent hover:border-slate-200 cursor-pointer"
            >
            ...
            {/* Notification Drawer */}
            {showNotifications && (
              <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-white rounded-2xl shadow-xl border border-slate-200 py-3 z-50 animate-fade-in text-slate-800">
                  ...
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((n) => (
                      ...
                      >
                        ...
                        {n.type === 'escalation' && <ShieldAlert className="w-4 h-4 text-rose-600 mt-0.5 flex-shrink-0" />}
          ...
          <div className="relative" ref={profileMenuRef}>
            ...
            {showProfileMenu && (
              <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-xl border border-slate-200 py-2 z-50 animate-fade-in text-slate-800">
                <div className="px-4 py-2.5 border-b border-slate-100">
                  <p className="text-xs font-black text-slate-900">{user?.name || 'Demo Merchant'}</p>
                  <p className="text-xs text-slate-500 font-mono truncate">{user?.email || 'demo@recoverx.ai'}</p>
                </div>
                ...
                <div className="py-1">
                  <button
                    ...
                    className="w-full px-4 py-2 text-left text-xs font-semibold text-slate-700 hover:bg-slate-50 hover:text-blue-600 flex items-center gap-2.5 cursor-pointer"
                  >
                    <User className="w-4 h-4 text-slate-400" /> Policy Guardrails
                  </button>
                </div>
      ...
      {toastMessage && (
        <div className="fixed bottom-6 right-6 z-50 bg-slate-900 text-white border border-blue-500 px-4 py-3 rounded-xl shadow-2xl text-xs font-bold flex items-center gap-2 animate-bounce">
