...
class PolicyEngine {
  ...
  async evaluatePolicy(txn, recommendation, customConfig = null) {
    ...
    if (!permittedActions.includes(proposedAction)) {
      approved = false;
      proposedAction = 'STOP';
      rejectionReason = `Proposed action '${recommendation.recommended_action}' is not in permitted policy allowlist.`;
    }

    // Guardrail Rule 2: Unrecoverable Failure Reason
    if (['card_expired', 'invalid_account'].includes(txn.failure_reason)) {
      approved = false;
      proposedAction = 'STOP';
    ...
    if (approved && (txn.retry_count || 0) >= maxRetries && ['SMART_RETRY', 'DELAYED_RETRY'].includes(proposedAction)) {
      approved = false;
      proposedAction = 'STOP';
      rejectionReason = `Transaction retry count (${txn.retry_count}) has reached maximum policy limit (${maxRetries}).`;
    }
    ...
    if (approved && txn.amount_inr >= highValueThreshold) {
      requiresHumanApproval = true;
      if (proposedAction !== 'HUMAN_ESCALATION') {
        proposedAction = 'HUMAN_ESCALATION';
        rejectionReason = `High-value transaction (₹${txn.amount_inr} >= ₹${highValueThreshold}) requires human approval.`;
      }
    }
